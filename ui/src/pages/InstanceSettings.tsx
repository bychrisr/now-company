...
export function InstanceSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("pages.instanceSettings.breadcrumbs.instanceSettings", { defaultValue: "Instance Settings" }) },
      { label: t("pages.instanceSettings.breadcrumbs.heartbeats", { defaultValue: "Heartbeats" }) },
    ]);
  }, [setBreadcrumbs, t]);
  ...
  const toggleMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("pages.instanceSettings.updateError", { defaultValue: "Failed to update heartbeat." }));
    },
  });
  ...
  const disableAllMutation = useMutation({
    mutationFn: async (agentRows: InstanceSchedulerHeartbeatAgent[]) => {
      ...
      if (failures.length > 0) {
        ...
        throw new Error(
          failures.length === 1
            ? t("pages.instanceSettings.disableTimerError.one", { detail, defaultValue: `Failed to disable 1 timer heartbeat: ${detail}` })
            : t("pages.instanceSettings.disableTimerError.other", { count: failures.length, total: enabled.length, detail, defaultValue: `Failed to disable ${failures.length} of ${enabled.length} timer heartbeats. First error: ${detail}` }),
        );
      }
      return enabled;
    },
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("pages.instanceSettings.disableAllError", { defaultValue: "Failed to disable all heartbeats." }));
    },
  });
    ...
    return (
      <div className="text-sm text-destructive">
        {heartbeatsQuery.error instanceof Error
          ...
          : t("pages.instanceSettings.loadError", { defaultValue: "Failed to load scheduler heartbeats." })}
      </div>
    );
  }
    ...
    <div className="max-w-5xl space-y-6">
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("pages.instanceSettings.title", { defaultValue: "Scheduler Heartbeats" })}</h1>
        </div>
        <p className="text-sm text-muted-foreground">
          {t("pages.instanceSettings.description", { defaultValue: "Agents with a timer heartbeat enabled across all of your companies." })}
        </p>
      </div>

      <div className="flex items-center gap-4 text-sm text-muted-foreground">
        <span><span className="font-semibold text-foreground">{activeCount}</span> {t("pages.instanceSettings.stats.active", { defaultValue: "active" })}</span>
        <span><span className="font-semibold text-foreground">{disabledCount}</span> {t("pages.instanceSettings.stats.disabled", { defaultValue: "disabled" })}</span>
        <span><span className="font-semibold text-foreground">{grouped.length}</span> {t(`pages.instanceSettings.stats.company`, { count: grouped.length, defaultValue: grouped.length === 1 ? "company" : "companies" })}</span>
        {anyEnabled && (
          <Button
            variant="destructive"
            size="sm"
            ...
            onClick={() => {
              const noun = enabledCount === 1 ? t("pages.instanceSettings.agent_one", { defaultValue: "agent" }) : t("pages.instanceSettings.agent_other", { defaultValue: "agents" });
              if (!window.confirm(t("pages.instanceSettings.confirmDisableAll", { count: enabledCount, noun, defaultValue: `Disable timer heartbeats for all ${enabledCount} enabled ${noun}?` }))) {
                return;
              }
      ...
      {agents.length === 0 ? (
        <EmptyState
          icon={Clock3}
          message={t("pages.instanceSettings.emptyState", { defaultValue: "No scheduler heartbeats match the current criteria." })}
        />
      ) : (
                  ...
                  {group.agents.map((agent) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          {agent.schedulerActive ? t("pages.instanceSettings.status.on", { defaultValue: "On" }) : t("pages.instanceSettings.status.off", { defaultValue: "Off" })}
                        </Badge>
                        <Link
                        ...
                        >
                          {agent.lastHeartbeatAt
                            ...
                            : t("pages.instanceSettings.lastHeartbeat.never", { defaultValue: "never" })}
                        </span>
                        <span className="ml-auto flex items-center gap-1.5 shrink-0">
                          <Link
                            to={buildAgentHref(agent)}
                            className="text-muted-foreground hover:text-foreground"
                            title={t("pages.instanceSettings.fullConfig", { defaultValue: "Full agent config" })}
                          >
